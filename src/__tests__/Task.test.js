...
describe('Task', () => {
    ...
    describe('.createAndRun()', () => {
        ...
        describe('with multiple requests', () => {
            test('.request and .requests', async () => {
                const test = new Test({}, {});
            ...
            test('.requests', async () => {
                ...
                expect(param.test_type).toBe(options.type);
                expect(param.urls).toBeInstanceOf(Array);
                expect(param.urls).toHaveLength(2);
                expect(param.urls[0]).toBeInstanceOf(Object);
                expect(param.urls[0].url).toBe('https://www.example.de/lol');
                expect(param.urls[1]).toBeInstanceOf(Object);
                expect(param.urls[1].url).toBe('https://www.example.de/narf');
            });
        });
    });
